...
describe("emitServiceRouters - Message Type Imports", () => {
	...
	it("uses message types directly in .input() and .output()", async () => {
		...
		// Should use message type names with protobuf validator
		expect(contents).toMatch(/\.input\(protobuf<\w+Request>\(\)\)/u);
		expect(contents).toMatch(/\.output\(protobuf<\w+Response>\(\)\)/u);
	});
	...
	it("handles services with multiple methods correctly", async () => {
		...
		const outputMatches = contents.match(/\.output\(protobuf</gu);
